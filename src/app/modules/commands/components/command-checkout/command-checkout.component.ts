...
export class CommandCheckoutComponent {
  ...
  constructor(
    ...
    this.produitService.getCart().then((res) => {
      this.cart = res;
      res.forEach((elt) => {
        if (!elt.product.productPromoPrice) {
          ...
          this.totalProductAmount +=
            elt.product.productPromoPrice * elt.quantity;
        }
      });
      this.totalOrderAmount =
        this.totalDeliveryAmount + this.totalProductAmount;
    });
    this.securityService.getAuthenticatedUser().then((res) => {
      if (res.userEmail) {
        ...
        if (res.userLastName) this.lastnameControl.setValue(res.userLastName);
  ...
  createCommand() {
    this.orderService
      .createOrder(this.hydrateNewOrderObject(), this.cart)
      .then((res: any) => {
        if (!res.order.orderReference) return;
        if (this.activePaymentMethod == 'paytech')
          return this.paymentsService.payWithPaytech(
            this.cart,
            this.totalOrderAmount,
            res.order.orderReference
          );
      });
  }
  refreshLinePercent() {
